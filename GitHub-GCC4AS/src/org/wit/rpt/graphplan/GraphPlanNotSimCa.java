...
/**
 * Only use capabilities in graph planning.
 * 
 * @author Guo Jingzhi
 */
public class GraphPlanNotSimCa {
	...
	static String fpccoo = "file:D://GCCmodel//CCmodel.owl";
	static String fpcco = "D:\\GCCmodel\\CCmodel.owl";
	...
	public static void drawGraph(ArrayList<ContextState> initialstates, ArrayList<ContextState> goalstates)
			throws IOException {
		File f = new File("D://GH-AGVsModel//ca_testfile.gv");
		if (f.exists()) {
			f.delete();
		}
		...
		System.out.println("GP(Ca)-notSim:");
	...
	public static void writeToFile(String s[]) throws IOException {
		File f = new File("D://GH-AGVsModel" + File.separator + "ca_testfile.gv");
		OutputStream out = new FileOutputStream(f, true);
